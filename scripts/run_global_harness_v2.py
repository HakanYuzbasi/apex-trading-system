...
logger = logging.getLogger("quant_system.global_harness_v2")
...
class StrategyController:
    ...
    def reconcile(self) -> None:
        ...
        for slot, config in self._configured_slots.items():
            ...
            logger.info("Activated Kalman strategy slot=%s pair=%s", slot, config.pair_label)
    ...
    def _instantiate_strategy(self, config: StrategySlotConfig) -> KalmanPairsStrategy:
        if config.strategy != "kalman_pairs":
            raise ValueError(f"Unsupported strategy in v2 harness: {config.strategy}")
    ...
    def _should_be_active(self, config: StrategySlotConfig) -> bool:
        ...
        if asset_class == AssetClass.CRYPTO:
            return True
        return self._session_manager.is_market_open(config.instrument_a) and self._session_manager.is_market_open(config.instrument_b)
    ...
    def reconcile(self, active_configs: tuple[StrategySlotConfig, ...]) -> None:
        ...
        for config in active_configs:
            ...
            if expected_sharpe is None:
                logger.warning("No expected Sharpe configured for %s; alpha monitor disabled for this pair", pair_label)
                continue
            ...
            logger.info("Alpha monitor activated for %s expected_sharpe=%.3f", pair_label, expected_sharpe)
...
async def _winner_refresh_loop(
    ...
    while not stop_event.is_set():
        ...
        if not startup_summary_sent:
            ...
            sharpe_lines = [
                f"{pair}: {expected_sharpes[pair]:.2f}"
                for pair in sorted(expected_sharpes)
            ]
            await notifier.notify_system_event(
                "Harness v2 Intelligence Online",
...
async def main() -> None:
    ...
    # Regime detector is intentionally instantiated before strategies so bar
    # events update risk state before any strategy reacts to new prices.
    regime_detector = RegimeDetector(event_bus)
    notifier = TelegramNotifier(event_bus)
    risk_manager = RiskManager(ledger, event_bus, regime_detector=regime_detector)
    ...
    winner_task = asyncio.create_task(
        ...
        name="winner-refresh-loop-v2",
    )
    stock_task = asyncio.create_task(stock_supervisor.run(stop_event), name="stock-stream-supervisor-v2")
    crypto_task = asyncio.create_task(crypto_supervisor.run(stop_event), name="crypto-stream-supervisor-v2")
    broker_task = asyncio.create_task(_run_broker_forever(broker, stop_event), name="alpaca-broker-supervisor-v2")

    logger.info(
        "Global harness v2 started cash=%.2f equity=%.2f recovered=%s",
        ledger.cash,
        ledger.total_equity(),
    ...
    try:
        ...
        logger.info("Stopping global harness v2")
        stop_event.set()
