      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 lg:gap-5">
          ...
          <div className="apex-panel rounded-2xl border border-border/80 px-4 py-3">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button size="sm" variant="outline" className="h-7 rounded-full px-3 text-xs" onClick={() => setDrawerOpen((v) => !v)}>
                  ...
                  triageAlerts.slice(0, 5).map((alert) => (
                    <div key={alert.id} className="flex flex-col gap-2 rounded-lg border border-border/70 bg-background/60 px-3 py-2 sm:flex-row sm:items-start sm:justify-between">
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold uppercase ${severityBadgeClass(alert.severity)}`}>
                            {alert.severity}
                          </span>
                          <span className="text-[11px] uppercase tracking-wide text-muted-foreground">{alert.source}</span>
                          <p className="text-sm font-semibold text-foreground">{alert.title}</p>
                        </div>
                        <p className="text-xs text-muted-foreground">{alert.detail}</p>
                      </div>
                      <Button size="sm" variant="outline" className="h-7 rounded-full px-3 text-xs" onClick={() => dismissAlert(alert.id)}>
                        Dismiss
                      </Button>
                    </div>
                  ))
        ...
        <section className="apex-panel apex-fade-up rounded-2xl p-4">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="text-[11px] text-muted-foreground">Core + USP operating signals</span>
          </div>
            ...
            {readinessItems.map((item) => (
              <div key={item.label} className="rounded-xl border border-border/70 bg-background/60 px-3 py-2">
                <div className="mb-1 flex items-center justify-between gap-2">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${readinessClass(item.state as "ok" | "warn" | "down")}`}>
                    {item.state}
                  </span>
                </div>
        ...
        <section className="grid grid-cols-1 gap-4">
          <article className="apex-panel apex-fade-up rounded-2xl p-5">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-lg font-semibold text-foreground">AI Mandate Copilot (Policy-Only)</h2>
                <p className="text-sm text-muted-foreground">
                  Goal-to-policy assessment with probability, confidence, DD/CVaR constraints, and no order generation.
                </p>
              </div>
            ...
            <div className="mt-4 rounded-xl border border-border/80 bg-background/60 p-3">
              ...
              {workflowPack ? (
                    ...
                    <Button
                      ...
                      onClick={() => {
                        void updateWorkflowStatus(
                          ...
                          "paper_live",
                          "Promoted from cockpit after PM + compliance sign-off.",
                        );
                      }}
                      disabled={workflowLoading || workflowPack.status !== "approved"}
                    >
                      Promote to paper_live
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  This creates a governance workflow pack only; it does not route orders into live trading.
                </p>
              )}
