...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const upstream = await fetch(`${getApiBase()}/api/v1/mandate-copilot/audit?limit=${encodeURIComponent(limit)}`, {
      method: "GET",
      cache: "no-store",
      headers: {
    ...
    const detail = error instanceof Error ? error.message : "Mandate audit proxy failed.";
