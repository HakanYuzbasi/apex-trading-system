from datetime import datetime
import pytz
from core.market_hours import is_market_open
from core.symbols import is_market_open as legacy_is_market_open

# Use .localize() instead of datetime(tzinfo=) to avoid pytz LMT offset bug
_ET = pytz.timezone("America/New_York")

def test_market_hours_equity():
    """Test equity market hours (NYSE/NASDAQ)."""
    # Monday 10:00 AM EST - OPEN (2023-10-23 is a Monday)
    dt_open = _ET.localize(datetime(2023, 10, 23, 10, 0, 0))

    assert is_market_open("AAPL", dt_open)
    assert legacy_is_market_open("AAPL", dt_open)

    # Monday 9:00 AM EST - CLOSED (before 9:30 open)
    dt_closed = _ET.localize(datetime(2023, 10, 23, 9, 0, 0))
    assert not is_market_open("AAPL", dt_closed)
    assert not legacy_is_market_open("AAPL", dt_closed)

    # Saturday - CLOSED
    dt_sat = _ET.localize(datetime(2023, 10, 21, 12, 0, 0))
    assert not is_market_open("AAPL", dt_sat)

def test_market_hours_crypto():
    """Test crypto market hours (24/7)."""
    # Sunday midnight
    dt = _ET.localize(datetime(2023, 10, 22, 0, 0, 0))

    assert is_market_open("BTC/USD", dt)
    assert is_market_open("CRYPTO:BTC/USDT", dt)

def test_market_hours_forex():
    """Test forex market hours."""
    # Tuesday 10 AM - OPEN
    dt_open = _ET.localize(datetime(2023, 10, 24, 10, 0, 0))
    assert is_market_open("FX:EUR/USD", dt_open)

    # Saturday noon - CLOSED
    dt_closed = _ET.localize(datetime(2023, 10, 21, 12, 0, 0))
    assert not is_market_open("FX:EUR/USD", dt_closed)

    # Sunday 4 PM EST - CLOSED (forex opens at 5 PM Sunday)
    dt_sun_closed = _ET.localize(datetime(2023, 10, 22, 16, 0, 0))
    assert not is_market_open("FX:EUR/USD", dt_sun_closed)

    # Sunday 6 PM EST - OPEN
    dt_sun_open = _ET.localize(datetime(2023, 10, 22, 18, 0, 0))
    assert is_market_open("FX:EUR/USD", dt_sun_open)
