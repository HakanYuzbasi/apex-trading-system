...
def test_heal_baselines_recovers_invalid_state():
    manager = RiskManager(max_daily_loss=0.02, max_drawdown=0.1)
    manager.starting_capital = 0.0
    manager.peak_capital = 0.0
    manager.day_start_capital = 0.0
...
def test_manual_reset_circuit_breaker_resets_latch_when_tripped():
    manager = RiskManager(max_daily_loss=0.02, max_drawdown=0.1)
